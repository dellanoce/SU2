...
def mesh_adaptation_amg( filename       ,
    ...
    # Set the name of the SU2_CFD configuration file
    config_cfd_name = "config_SU2.cfg"
    
    # Read the specified configuration file
    config = SU2.io.Config(config_name)
    config_cfd = SU2.io.Config(config_cfd_name)
    
    # Set the number of partitions for parallel computations
    config.NUMBER_PART = partitions
    config_cfd.NUMBER_PART = partitions
        
    # Call CFD to generate a solution
    SU2.run.amg(config, config_cfd, stderr)
